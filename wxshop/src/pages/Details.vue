      ...
      <div class="inner" v-if="!tips1">
        <div class="details-hd">
          ...
          <div class="details-mes">
            <div class="details-title"><span class="y_c" v-if="detailsData.corpId == 'youdaoauto'">友道会</span><span>{{detailsData.prodTitle}}</span></div>
            <div class="details-other">
              <div class="details-price">
                ...
                <span class="details-del-price">原价￥{{detailsData.orgPrice}}</span>
              <!-- 秒杀活动未开始 -->
        ...
        <div class="details-action-menu">
          <p class="action-appointment-ing" v-if="detailsData.memberPrice-detailsData.pointPrice>0&&detailsData.memberPoint>0"><span class="action-mess no-mess s_1">积分兑现金</span><span>{{detailsData.memberPoint}}积分可兑换{{detailsData.memberPrice-detailsData.pointPrice}}元现金</span></p>

          <p class="action-appointment-ing" v-if="reducedInfo !=null"><span class="action-mess no-mess s_2">满减送</span><span>满{{reducedInfo.threshold}}送{{reducedInfo.reducedMoney}}</span></p>
          <!-- 秒杀中、秒杀结束限购提示 -->
        ...
        <div class="details-ft" v-if="detailsData.serveCode == ''">
          <div class="buy-bar">
        ...
        <div class="service_btn" @click="serviceBuy" v-else>立即购买</div>
      ...
      <div class="tipsPage" v-if="tips1">
       ...
       <!-- 商品属性弹窗 -->
        ...
        <yd-bottom-tip v-if="tips4" :isShow='false' :tipsType='"tips4"'>
         ...
         <!--权益服务立即购买弹窗-->
        <yd-bottom-tip v-if="show2" :tipsType='"serviceShow"'>
...
export default {
  ...
  methods: {
    ...
    addCart() {
      ...
      this.log(LOGCODE.HOME.BTN_PAGE_DETAIL_CART_ADD);//加入购物车
    },
    //设置积分抵扣价是否有小数点
    setPointPrice(){
    ...
    buy(isContinue) {
      this.log(LOGCODE.HOME.BTN_PAGE_DETAIL_BUY);
    ...
    reserveBtn(str) {
      let that = this;
        this.log(LOGCODE.HOME.BTN_PAGE_DETAIL_RESERVE);
        ...
        .then((response) => {
          ...
          if(data.code == "S_OK") {
            ...
            that.log(LOGCODE.HOME.MESS_PAGE_DETAIL_RESERVE_OK);
          }else{
            if(data.code == "LOGIN_SID_FAL_0X001" || data.code == "LOGIN_FAL_0x003" || data.code == "LOGIN_FAL_0x004" || data.code == "NO_LOGIN") {
              that.$dialog.notify({mes: '登录失效，请重新登录', icon: 'error', timeout: 1200, callback: () => {
                that.$router.push('/login');
              ...
              that.log(LOGCODE.HOME.MESS_PAGE_DETAIL_RESERVE_FAIL);
              that.$dialog.notify({mes: codeKeyValue[data.code], icon: 'error', timeout: 1200});
            }
          }
        },(response) => {
