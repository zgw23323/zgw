<template>
    ...
    <article class="content s_contract serviceCity">
      <div v-if="opt.carsName !=''" class="car_fixed_mess">当前选择的车系是<span>{{opt.carsName}}</span></div>
      <div :class="{'cart_seach':opt.carsName =='','cart_seach c_padd':opt.carsName !=''}">
        <div>
          <i class="iconfont">&#xe602;</i>
      ...
      <div class="s_city" v-for="item in carData" v-if="item.carsList && item.carsList.length>0">
        ...
        <ul>
          <li :id="car.carsId" v-for="(car,index) in item.carsList" :class="{'no_bor flex_between':index==item.carsList.length-1,'flex_between':index!=item.carsList.length-1}" @click="carBtn(car)">{{car.carsName}} <i class="car_gou" v-if="opt.carsId == car.carsId">&nbsp;</i></li>
        </ul>
      </div>
    </article>
  </section>
  ...
  export default {
    ...
    methods: {
      ...
      markInit() {
        ...
        //获取shopsid缓存数据
        setShopsId();
        var carOpt = getSessionStore('addCar_info',true);
      ...
      carBtn(opt){
        this.opt.carsName = opt.carsName;
        this.opt.carsId = opt.carsId;
        setSessionStore('addCar_info',this.opt);
        this.$router.push({
