              ...
              <div class="file gong">
                ...
                <div class="upImg_gong" v-if="isCostFileShowBtn">
                  ...
                  <form method="post" enctype="multipart/form-data" id="form" class="form" action="upload">
                      <div class="p_20">上传计费报价单</div>
                      <a class="file-input" id="costFile"></a>
                  </form>
                </div>
              </div>
            </div>
              ...
              <div class="file gong">
                ...
                <div class="upImg_gong" v-if="isAgreementFileShowBtn">
                  ...
                  <form method="post" enctype="multipart/form-data" class='form' action="upload">
                      <div class="p_20">上传服务协议单</div>
                      <a class="file-input" id="agreementFile"></a>
                  </form>
                </div>
              </div>
            </div>
  ...
  export default {
    ...
    mounted() {
      ...
      ajaxModel.generateToken(this,(result)=>{
        that.openToken = result.token;
        that.qiniuToken = result.qiniuToken;
        that.newUploadFileFun(['costFile','agreementFile']);
      },'?tokenType=qiniu')
    ...
    methods: {
      ...
      newUploadFileFun(arr){
        ...
        for(var i=0;i<arr.length;i++){
          ...
          qiniuLib.uploadWithSDK1(that.qiniuToken,arr[i],this,(key,strId) =>{
            ...
            if(strId == 'agreementFile'){
              that.optApply.agreementFileId = window.qiniuDomain+key;
              that.imgList.agreementFile.push(window.qiniuDomain+key);
              that.isAgreementFileShowBtn=false;
            }else if(strId == 'costFile'){
              that.optApply.costFileId = window.qiniuDomain+key;
              that.imgList.costFile.push(window.qiniuDomain+key);
          ...
          .then((response) => {
            ...
            if(data.code == "S_OK") {
              if(data.var){
                ...
                if(data.var.costFileUrl){
                  var _costFileUrl = data.var.costFileUrl.split('http').length>1 ? data.var.costFileUrl:that.imgBaseUrl+data.var.costFileUrl;
                  that.imgList.costFile.push(_costFileUrl);
                  that.isCostFileShowBtn=false;
                }
                if(data.var.agreementFileId){
                  var _agreementFileUrl = data.var.agreementFileUrl.split('http').length>1 ? data.var.agreementFileUrl:that.imgBaseUrl+data.var.agreementFileUrl;
                  that.imgList.agreementFile.push(_agreementFileUrl);
                  that.isAgreementFileShowBtn=false;
                }
      ...
      deleteImg(fileId,str){
        ...
        if(str == 'costFile'){
          ...
          // this.newUploadFileFun(['costFile']);
        }
        if(str == 'agreementFile') {
          this.isAgreementFileShowBtn = true;
          // this.newUploadFileFun(['agreementFile']);
        }
        setTimeout(()=>{
          that.newUploadFileFun([str]);
        },200);
      },
      tiemFocus(n){
      ...
      submit(str){
        ...
        if(that.optApply.costFileId == ''){
          ...
          that.optApply.costFileId = that.optApply.costFileId;
        }
        if(that.optApply.agreementFileId == ''){
          that.$dialog.notify({mes: '上传服务协议照片', icon: 'error', timeout: 1200});
          return;
        }else{
          that.optApply.agreementFileId = that.optApply.agreementFileId;
        }
        if(this.isApply){
          return false;
        }else{
        ...
        .then((response) => {
          ...
          if(data.code == "S_OK") {
            ...
            that.$store.dispatch('log', {account:LOGCODE.WORKERPASS.MESS_PAGE_GLASSOPENORDER_SAVE_OK});
          }else {
            this.isApply = false;
            that.$store.dispatch('log', {account:LOGCODE.WORKERPASS.MESS_PAGE_GLASSOPENORDER_SAVE_FAIL});
      ...
      readFile(str){ 
        ...
        console.log('document.getElementById(str)',document.getElementById(str),str);
        if(file){
          this.tips4 = true;
          this.$dialog.loading.open('上传中...');
          qiniuLib.uploadWithSDK(file,this.qiniuToken,(key,domain)=>{
            this.tips4 = false;
            console.log('domain',domain+key);
            if(str == 'agreementFile'){
